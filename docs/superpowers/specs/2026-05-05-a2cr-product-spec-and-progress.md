...
| RLS isolation | user Aがuser Bのdataを読めないことをテスト | 静的テスト + ローカルPostgres実DB検証済み。API key routeのDB smoke test済み。Dashboard/MCP統合は未実装 |
...
| Web SaaS実装 | 一部着手 | FastAPI security foundationとWorkBaton Web Context APIを追加済み。HTTP MCPとReact dashboardは未実装 |
